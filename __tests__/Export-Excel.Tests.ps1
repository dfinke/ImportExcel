...
Describe ExportExcel {
    ...
    Context "                # ToExcel method" {
        it "Should find the ToExcel method" {
            (Get-TypeData  -TypeName array).Members.GetEnumerator() |
            Where-Object { $_.key -ceq 'ToExcel' } | Should -Not -BeNullOrEmpty
        }

        it "Should export correctly with ToExcel" {

            $data = ConvertFrom-Csv @"
Region,Item,TotalSold
North,saw,76
South,screwdriver,93
East,lime,44
West,peach,80
East,saw,77
North,screws,93
...
"@
            ...
            $actual = Import-Excel -Path $path -WorksheetName Sheet1
