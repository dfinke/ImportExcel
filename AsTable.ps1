...
Performs type conversions on the properties of a list of PSCustomObject's.
Typically used to make a table where every property is guaranteed to be of a
certain type.
...
PS> $csvData | .\AsTable.ps1 -InterpretStrings

This will interpret strings in $csvData and convert them to their closest
matching type. Non-string types will be converted to [double] or [datetime]
(unless of course they are already of that type), or if that fails, to
[string]. Not really a table, since columns can now contain data of various
types. Alternatively, non string types can just be passed through the pipeline
without alteration.

.EXAMPLE

PS> $csvData | .\AsTable.ps1 -PropertyTypes @{ ID=[string]; Quantity=[double] }

This will ensure that all the ID property values will be of type [string], and
all Quantity property values will be of type [double]. This is the same as
saying, the ID column will contain strings, etc.

.EXAMPLE

PS> $csvData | .\AsTable.ps1 -GlobalType ([string])

This will ensure that every property value will be of type [string]. This
amounts to a table containing only string values.
